...
export default function Biblioteca({games}: LibraryProps) {
    ...
    const [userSteamGames, setUserSteamGames] = useState<string[] | null | undefined>(null);
    const [cardsSteamGames, setCardSteamGames] = useState<{ [key: string]: any } | null>();
    ...
    function loadUserGames() {
        if (localStorage.getItem("userGames")) {
            ...
            const userSteamGames = JSON.parse(localStorage.getItem("userSteamGames") || "{}");
            ...
            getSteamGames().then(r => {
                localStorage.setItem("userSteamGames", JSON.stringify(r.games.map((game: any) => game)));
    ...
    return (
        <div className="bg-blue-jeans-50 min-h-screen">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                            ...
                            <div key={gameId} className="rounded p-2 relative top-2">
                                {cardsGames && cardsGames[gameId] && cardsGames[gameId].cover && (
                                    ...
                                    <div className="text-center relative mx-auto">
                                        <div
                                            className={`top-0 left-0 w-fit h-fit ${pickColorForCard(gameId)} rounded border-2  border-solid`}>
                                            {/* Barra superior */}
                                            <div
                                                className={`w-full h-8 sm:h-10 ${pickColorForCard(gameId)} flex items-center justify-between rounded-t`}>
                                                {/* Título de status */}
                ...
                {userSteamGames && userSteamGames.length > 0 && cardsSteamGames && (
                    userSteamGames.slice(0, displayedGames).map((game: any) => (
                        <div key={game.appid} className="rounded p-2 relative top-2">
                            {cardsSteamGames && cardsSteamGames[game._id] && cardsSteamGames[game._id].cover && (
                                <div className="text-center relative mx-auto">
                                    <div
                                        className={`top-0 left-0 w-fit h-fit ${pickColorForCard(game._id)} rounded border-2  border-solid`}>
                                        {/* Barra superior */}
                                        <div
                                            className={`w-full h-8 sm:h-10 ${pickColorForCard(game._id)} flex items-center justify-between rounded-t`}>
                                            {/* Título de status */}
                                            <div className="text-white text-sm py-1 px-2 rounded-t  ">
                                            ...
                                            <div
                                                className="right-0 text-white py-1 px-2 rounded cursor-pointer items-center">
                                                <CardModal id={game._id} forceReload={forceReloadOnGameDelete}
                                                           forceReloadOnChange={forceReloadOnGameStatusChange}/>
                                            </div>
                                        </div>
                            ...
                            <h2 className="text-lg mt-2 text-center text-white">
                                {cardsSteamGames && cardsSteamGames[game._id] && cardsSteamGames[game._id].name}
                            </h2>
                        </div>
                    ))
                )}
            ...
            <div className="flex justify-center p-6">
                <button onClick={() => setDisplayedGames(displayedGames + 10)} className="bg-blue-jeans-500 hover:bg-blue-jeans-600 text-white font-bold py-2 px-4 rounded mt-4">
                    Carregar mais
                </button>
            </div>
